...
str(states)
names(states)  # "Murder" : 살인사건발생률 "Population" : 인구수 "Illiteracy" : 문맹율 "Income"  : 수입   "Frost" : 결빙일
...
install.packages("car", dependencies = T)  # car의 종속적인 모든 패키지 설치
library(car)
...
# Population Illiteracy Income Frost  살인사건 발생율 예측값
...
#       df      AIC
# mfit1  6 241.6429
# mfit2  4 237.6565 값이 작은 것이 좋은 모델
...
library(leaps)
m <- regsubsets(Murder ~ Population + Illiteracy + Income + Frost, data = states, nbest = 4)
summary(m)
