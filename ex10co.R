...
cov_xy <- cov(hf_man$father, hf_man$height)  # 공분산 2.368441. 관계의 방향을 알 수 있으나 관계의 정도를 정확히 알 수 없다.
cov_xy
# 상관계수는 공분산을 각 변수의 표준편차의 곱으로 나눔
r_xy <- cov_xy / (sd(hf_man$father) * sd(hf_man$height))
r_xy  # 상관 계수

cor(hf_man$father, hf_man$height)  # 0.3913174  아빠키가 1단위 증가하면 아들키는 0.39 단위정도로 증가한다
