# Logistic Regression - 이진분류(시그모이드 함수를 사용)
# 상관/인과관계 x
# 독립변수 : 연속형, 종속변수 : 범주형

# 연습1 : binary.csv : 영어 성적 등으로 대학원 입학여부 분류 모델 작성
...
# 모델
lgmodel <- glm(formula = admit ~ ., data = train, family = binomial(link = 'logit'))  # 이항분포 
lgmodel 
...
cat('예측값 : ', head(pred, 30))
cat('예측값 : ', head(ifelse(pred > 0.5, 1, 0), 30))
cat('실제값 : ', test$admit[1:30])
...
new_perd <- predict(lgmodel, newdata = new_data, type = "response")
new_perd
ifelse(new_perd > 0.5, '합격', '불합격')
